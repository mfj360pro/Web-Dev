...
class EditorController extends WebService
{
    public function __construct() {
        $this->middleware(function ($request, $next) {
            if(Session::has('app_editor_id')) {
                $data = json_decode($this->ReadAccount(new Request(['id' => Session::get('app_editor_id')])), true);
                View::share('data', $data);
                View::share('menus', json_decode($this->ReadMenusByRole(new Request(['role_id' => 7])), true));
                View::share('accesstype', 'editor');
                View::share('accessname', 'Editor Portal');
                View::share('dashboard_msg', 'You have successfully logged in to the editors panel.');
                return $next($request);
            }
            else return redirect(action('LoginController@EditorLogin'));
        });
    }
    ...
    public function Details(Request $request) {
        ...
        return view('editor.pages.details', 
    ...
    public function Quotations() {
        ...
        return view('editor.pages.quotations', 
