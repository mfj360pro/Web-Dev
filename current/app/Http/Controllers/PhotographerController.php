...
class PhotographerController extends WebService
{
    public function __construct() {
        $this->middleware(function ($request, $next) {
            if(Session::has('app_photographer_id')) {
                $data = json_decode($this->ReadAccount(new Request(['id' => Session::get('app_photographer_id')])), true);
                View::share('data', $data);
                View::share('menus', json_decode($this->ReadMenusByRole(new Request(['role_id' => 5])), true));
                View::share('accesstype', 'photographer');
                View::share('accessname', 'Photographer Portal');
                View::share('dashboard_msg', 'You have successfully logged in to the photographers panel.');
                return $next($request);
            }
            else return redirect(action('LoginController@PhotographerLogin'));
        });
    }
    ...
    public function Details(Request $request) {
        ...
        return view('photographer.pages.details', 
    ...
    public function Quotations() {
        ...
        return view('photographer.pages.quotations', 
