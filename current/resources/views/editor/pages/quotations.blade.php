        ...
        $(function() {
            ...
            $(grid_available_shoots).jsGrid({
                ...
                controller: {
                    loadData: function() {
                        var d = $.Deferred();
                        Controller.Create('/WebService/ReadShootsByStatusAndCompletion', { 'status_id' : "3", 'completed': 1 }).done(function(data) {
                            d.resolve(data);
                        });
                        return d.promise();
                    },
                    updateItem: function(item) {
                        Controller.Update('/WebService/UpdateShootStatusAndHandler', item).done(function(data) {
            ...
            $(grid_my_shoots).jsGrid({
                ...
                controller: {
                    loadData: function() {
                        var d = $.Deferred();
                        Controller.Create('/WebService/ReadShootsByAccount', { 'id' : "{{$data['id']}}" }).done(function(data) {
                            d.resolve(data);
                        });
                        return d.promise();
                    },
                    ...
                    [
                        ...
                        {   name: "action",         type: "text",       width: 50,  title: "Action",
                            itemTemplate: function(value, item) {
                                ...
                                if(item.current_status == 4 && item.completed == 0) {

                                    var iconMark = $('<span>').text('Mark As DONE');
                                    var btnMark  = $("<button>").addClass('btn btn-sm btn-success')
                                                    .append(iconMark)
                                                    .click(function(e) {
                                                        Controller.Create('/WebService/MarkShootAsReady', {'id':item.id}).done(function() {
                                                            $(grid_my_shoots).jsGrid("loadData");
                                                        });
                                                    });
